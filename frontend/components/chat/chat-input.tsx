...
export function ChatInput({
  ...
  const updateText = (value: string) => {
    ...
    mentions.detect(value, el?.selectionStart ?? value.length);
  };

  const pickMention = (member: TeamMember) => {
    const el = textareaRef.current;
    const caret = el?.selectionStart ?? text.length;
    const next = mentions.insert(text, caret, member);
    setText(next.text);
    el?.focus();
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const mentionUserIds = content ? mentions.resolve(content).map((m) => m.userId) : [];
    onSend({
      content,
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (mentions.open && mentions.candidates.length > 0 && (e.key === "Tab" || e.key === "Enter")) {
      e.preventDefault();
      pickMention(mentions.candidates[0]);
      return;
    }
  ...
  return (
    ...
    >
      <div className="absolute bottom-full left-2 mb-1">
        <MentionSuggestions candidates={mentions.candidates} onPick={pickMention} />
      </div>
